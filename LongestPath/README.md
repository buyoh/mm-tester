...
<h3>問題文</h3>
座標が重複しない$N$個の平面上の頂点，頂点同士を繋ぐ$M$個の辺からなるグラフが与えられます．頂点には$0$から$N-1$の番号が振られており，$i$番目の頂点の座標は$(x_{i}, y_{i})$です．辺には$0$から$M-1$の番号が振られており，$j$番目の辺は頂点$a_{j}$と$b_{j}$を繋ぎます．多重辺や自己ループはありません．また辺の重みはユークリッド距離です．なるべく距離の長い単純路を見つけてください．
